...
class CentroMejora {
   ...
   */
  openPlaylistModal(playlistCard) {
    const playlistIndex = Array.from(this.playlistsGrid.children).indexOf(playlistCard)
    const playlist = this.allPlaylists[playlistIndex]

    if (playlist && playlist.videos.length > 0) {
      this.openVideoModal(playlist.videos[0])
    }
  }
