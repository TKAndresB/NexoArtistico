...
class CentroMejora {
  ...
  createPlaylistView(playlistData) {
      ...
      <div class="playlist-view">
        <div class="playlist-header">
          ...
          <div class="playlist-actions">
            <button class="btn btn-primary complete-playlist-btn" id="complete-playlist-btn">
              <i class="fas fa-check-double"></i>
              Marcar Playlist Completa
            </button>
          </div>
        </div>
    ...
    const completePlaylistBtn = modalBody.querySelector("#complete-playlist-btn")
    if (completePlaylistBtn) {
      completePlaylistBtn.addEventListener("click", () => {
        this.markPlaylistCompleted(playlistData)
      })
    }
  }
  ...
  /**
   * Marca toda la playlist como completada
   * @param {Object} playlistData - Datos de la playlist
   */
  markPlaylistCompleted(playlistData) {
    // Marcar todos los videos de la playlist como completados
    ...
    // Actualizar todas las tarjetas de video
    playlistData.videos.forEach((video) => {
      this.updateVideoCardState(video.id, true)
    })

    // Recrear la vista de la playlist para mostrar los cambios
    this.createPlaylistView(playlistData)

    // Mostrar mensaje de confirmación
    const completeBtn = document.querySelector("#complete-playlist-btn")
    if (completeBtn) {
      const originalText = completeBtn.innerHTML
      completeBtn.innerHTML = '<i class="fas fa-check"></i> ¡Playlist Completada!'
