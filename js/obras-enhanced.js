...
document.addEventListener("DOMContentLoaded", () => {
  console.log("Script de obras mejorado cargado correctamente")
...
function initAdvancedFilters() {
  ...
  filterToggle?.addEventListener("click", () => {
    ...
    filterToggle.innerHTML = advancedFilters?.classList.contains("active")
  ...
  function applyFilters() {
    ...
    memberCards.forEach((card) => {
      ...
      const memberSince = card.querySelector(".member-since")?.textContent.toLowerCase() || ""

      // Obtener técnicas de las obras
  ...
  function updateCounters(visibleCount) {
    ...
    // Contar obras visibles
    let visibleWorks = 0
    memberCards.forEach((card) => {
      if (card.style.display !== "none") {
        const worksCount = card.querySelector(".btn-view-works")?.textContent.match(/\d+/)
        if (worksCount) {
    ...
    if (totalWorks) {
      totalWorks.textContent = visibleWorks
    }
  }
}
